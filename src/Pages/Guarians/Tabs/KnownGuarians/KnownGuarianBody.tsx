...
const KnownGuarianBody: React.FC<IProps> = ({ forceOpen }) => {
  const [isCollapsed, setCollapsed] = useState(true);

  function renderHeader() {
    if (forceOpen) {
      return <h1>Known Guarians</h1>;
    }

    return (
      <h2 className="pressable-header" onClick={() => setCollapsed(!isCollapsed)}>
        <FontAwesomeIcon
          icon={faChevronRight}
  ...
  return (
    <div>
      {renderHeader()}
      <Collapse isOpen={!isCollapsed || forceOpen}>
        <p>
        ...
        <p>
          In short, the following attributes are written from my own personal perspective and may
          vary heavily depending on your own interpretation of each guarian:
        </p>
        <ul style={{ marginTop: -10 }}>
          ...
          <li style={{ marginTop: 6 }}>
            ...
            manifests itself is constructed around the preferences of their beholder, and as such,
            the sizes and materials I write about may be completely irrelevant to how they might
            appear to you.
          </li>
        </ul>
      </Collapse>
